...
def main(
    ...
    """
    对通量模型进行采样。可以采用交互式方式（设置 --loop 参数），也可以仅针对单张图像运行采样。
参数说明：
height：采样图像的高度（以像素为单位，应为 16 的倍数）
width：采样图像的宽度（以像素为单位，应为 16 的倍数）
seed：设置采样的随机种子
output_name：保存输出图像的位置，{idx} 将被采样的索引所替换
prompt：用于采样的提示词
device：PyTorch 设备
num_steps：采样步骤的数量（默认为 4，适用于 schnell 方法；50 适用于引导蒸馏方法）
loop：启动交互式会话并进行多次采样
guidance：用于引导蒸馏的引导值
add_sampling_metadata：将提示词添加到图像的 Exif 元数据中
img_cond_path：条件图像的路径（支持 jpeg/png/webp 格式）
trt：使用 TensorRT 后端进行优化推理
    """
